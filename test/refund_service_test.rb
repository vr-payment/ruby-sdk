...
class TestRefundService < Test::Unit::TestCase
  ...
  class << self
    def startup
      ...
      @@refund_service = VRPayment::RefundService.new
    end
  end
  ...
  def test_refund
    ...
    assert_equal(VRPayment::RefundState::SUCCESSFUL, refund.state,
        "State must be SUCCESSFUL")
  end
  ...
  def test_search
    ...
    criteria = VRPayment::EntityQuery.new({
  ...
  def get_refund_create(transaction)
    VRPayment::RefundCreate.new({
        externalId: transaction.id.to_s,
        type: VRPayment::RefundType::MERCHANT_INITIATED_ONLINE,
