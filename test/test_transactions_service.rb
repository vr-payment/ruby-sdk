...
# vrpayment transactions service tests
class TransactionsServiceTest < Test::Unit::TestCase

  @@INTEGRATION_MODE = 'payment_page';

  def setup
    @transactions_service = VrpaymentRubySdk::TransactionsService.new()
    @tokens_service = VrpaymentRubySdk::TokensService.new()
  end
  ...
  def test_create_and_find_pending_transaction
    ...
    assert_equal(
      VrpaymentRubySdk::TransactionState::PENDING,
      transaction.state,
  ...
  def test_confirm_should_make_transaction_confirmed
    ...
    transaction_pending = VrpaymentRubySdk::TransactionPending.new(version: 2)
    ...
    assert_equal(
      VrpaymentRubySdk::TransactionState::CONFIRMED,
      confirmed.state,
  ...
  def test_process_deferred_transaction_should_make_transaction_authorized
    transaction_create = TestUtils.get_transaction_create_payload
    transaction_create.tokenization_mode = VrpaymentRubySdk::TokenizationMode::FORCE_CREATION
    transaction_create.customers_presence = VrpaymentRubySdk::CustomersPresence::NOT_PRESENT
    transaction_create.completion_behavior = VrpaymentRubySdk::TransactionCompletionBehavior::COMPLETE_DEFERRED
    ...
    assert_equal(
      VrpaymentRubySdk::TransactionState::AUTHORIZED,
      processed.state,
  ...
  def test_process_via_charge_flow_should_make_transaction_processing
    ...
    assert_equal(
      VrpaymentRubySdk::TransactionState::PROCESSING,
  ...
  def test_cancel_charge_flow_should_make_transaction_failed
    ...
    assert_equal(
      VrpaymentRubySdk::TransactionState::FAILED,
  ...
  def test_complete_online_should_make_transaction_completion_state_successful
    ...
    transaction_create.tokenization_mode = VrpaymentRubySdk::TokenizationMode::FORCE_CREATION
    transaction_create.customers_presence = VrpaymentRubySdk::CustomersPresence::NOT_PRESENT
    transaction_create.completion_behavior = VrpaymentRubySdk::TransactionCompletionBehavior::COMPLETE_IMMEDIATELY
    ...
    assert_equal(
      VrpaymentRubySdk::TransactionCompletionState::SUCCESSFUL,
    ...
    assert_equal(
      VrpaymentRubySdk::TransactionState::FULFILL,
  ...
  def test_complete_online_partially_should_make_transaction_completion_state_successful
    ...
    tcd = VrpaymentRubySdk::TransactionCompletion.new
  ...
  def test_void_transaction_online_should_return_voided_transaction
    ...
    assert_equal(
      VrpaymentRubySdk::TransactionVoidState::SUCCESSFUL,
    ...
    assert_equal(
      VrpaymentRubySdk::TransactionState::VOIDED,
  ...
  def test_fetch_with_credentials_with_given_bad_credentials_should_fail
    assert_raises(VrpaymentRubySdk::ApiError, "Bad token should error response") do
  ...
  def test_update_should_change_transaction_data
    ...
    update = VrpaymentRubySdk::TransactionPending.new(version: 2, language: "en-GB")
  ...
  def test_process_one_click_token_and_redirect_with_credentials_should_return_payment_url
    ...
    token_create = VrpaymentRubySdk::TokenCreate.new(external_id: SecureRandom.uuid)
    token_create.state = VrpaymentRubySdk::CreationEntityState::ACTIVE
    ...
    token_update = VrpaymentRubySdk::TokenUpdate.new(enabled_for_one_click_payment: true, version: token.version)
    ...
    line_item = VrpaymentRubySdk::LineItemCreate.new(
      name: 'Red T-Shirt',
      unique_id: '5412',
      type: 'PRODUCT',
      quantity: 1,
    ...
    transaction1_pending = VrpaymentRubySdk::TransactionPending.new(
    ...
    transaction2_create.tokenization_mode = VrpaymentRubySdk::TokenizationMode::FORCE_CREATION_WITH_ONE_CLICK_PAYMENT
    transaction2_create.customers_presence = VrpaymentRubySdk::CustomersPresence::NOT_PRESENT
    transaction2_create.completion_behavior = VrpaymentRubySdk::TransactionCompletionBehavior::COMPLETE_DEFERRED
    ...
    assert_not_equal(read_transaction.state, VrpaymentRubySdk::TransactionState::FAILED, "Transaction state should not be FAILED")
    assert_not_equal(read_transaction.state, VrpaymentRubySdk::TransactionState::PENDING, "Transaction state should not be PENDING")

    completed_transaction1 = @transactions_service.post_payment_transactions_id_complete_online(
      transaction1.id, SPACE_ID
    )

    assert_equal(completed_transaction1.state, VrpaymentRubySdk::TransactionCompletionState::SUCCESSFUL, "State must be SUCCESSFUL")
  end
  ...
  def test_process_transaction_via_token_should_process_transaction
    ...
    transaction_pending = VrpaymentRubySdk::TransactionPending.new(
      token: token.id,
    ...
    assert_equal(read_transaction.state, VrpaymentRubySdk::TransactionState::FULFILL, "Transaction state must be FULFILLED")
  end
  ...
  def test_process_without_user_interaction_should_process_transaction_properly
    ...
    assert_equal(processed.state, VrpaymentRubySdk::TransactionState::PROCESSING, "Transaction state should be PROCESSING")
  end
  ...
  def test_process_transaction_with_three_d_secure_should_fulfill_transaction
    transaction = create_transaction(TestUtils.get_transaction_create_payload)
    tokenized_card_data_create = VrpaymentRubySdk::TokenizedCardDataCreate.new(primary_account_number: MOCK_CARD_DATA.card_data.primary_account_number)
    tokenized_card_data_create.expiry_date = MOCK_CARD_DATA.card_data.expiry_date

    tcr = VrpaymentRubySdk::TokenizedCardRequest.new(card_data: tokenized_card_data_create)
    ...
    assert_not_nil(url, "Url should not be null")
    assert_not_equal(url, "", "Url should not be empty")
    assert_not_equal(processed.state, VrpaymentRubySdk::TransactionState::FAILED, "Transaction state should not be FAILED")
    assert_not_equal(processed.state, VrpaymentRubySdk::TransactionState::PENDING, "Transaction state should not be PENDING")
  end
  ...
  def test_fetch_terminal_receipts_should_return_valid_terminal_receipts_list
    ...
    receipt_response = @transactions_service.get_payment_transactions_id_terminal_receipts(
      transaction.id, VrpaymentRubySdk::TerminalReceiptFormat::TXT, 200, SPACE_ID
    )
