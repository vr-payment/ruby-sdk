...
# vrpayment webhook listeners service tests
class WebhookListenersServiceTest < Test::Unit::TestCase
  ...
  def setup
    @webhook_listeners_service =   VrpaymentRubySdk::WebhookListenersService.new
    @webhook_urls_service =   VrpaymentRubySdk::WebhookURLsService.new
  end
  ...
  def test_create_and_delete_webhook_listener
    ...
    assert_equal(  VrpaymentRubySdk::CreationEntityState::DELETED, deleted_response.state, "State should be DELETED")
  end
  ...
  def test_get_webhooks_listeners_search_without_custom_timeout
    ...
    search_response.data.each do |webhook_listener|
      ...
      assert_equal   VrpaymentRubySdk::CreationEntityState::INACTIVE, webhook_listener.state,
                   "Expected Webhook Listener State #{webhook_listener.state} to equal #{   VrpaymentRubySdk::CreationEntityState::INACTIVE}"
    end
  end
  ...
  def test_get_webhooks_listeners_search_with_custom_timeout
    ...
    config_with_timeout =   VrpaymentRubySdk::Configuration.new do |c|
      c.user_id = TEST_USER_ID
      c.authentication_key = TEST_AUTHENTICATION_KEY
      c.timeout = 36
    end
    api_client_with_timeout =   VrpaymentRubySdk::ApiClient::new(config_with_timeout)
    webhook_listener_service_with_timeout =   VrpaymentRubySdk::WebhookListenersService.new(api_client_with_timeout)
  ...
  def private_webhook_listener_create(webhook_url_id)
      VrpaymentRubySdk::WebhookListenerCreate.new(
      {
        name: WEBHOOK_LISTENER_NAME,
        state:   VrpaymentRubySdk::CreationEntityState::ACTIVE,
