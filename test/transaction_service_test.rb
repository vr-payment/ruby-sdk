...
class TestTransactionService < Test::Unit::TestCase
  ...
  class << self
    def startup
      ...
      @@card_processing_service = VRPayment::CardProcessingService.new

      @@transaction_completion_service = VRPayment::TransactionCompletionService.new

      @@token_service = VRPayment::TokenService.new
    end
  end
  ...
  def test_create
    ...
    assert_equal(VRPayment::TransactionState::PENDING, transaction.state,
  ...
  def test_confirm
    ...
    assert_equal(VRPayment::TransactionState::CONFIRMED, transaction_confirmed.state,
  ...
  def test_create_complete_deferred
    ...
    transaction_create.tokenization_mode = VRPayment::TokenizationMode::FORCE_CREATION
    transaction_create.customers_presence = VRPayment::CustomersPresence::NOT_PRESENT
    transaction_create.completion_behavior = VRPayment::TransactionCompletionBehavior::COMPLETE_DEFERRED
    ...
    assert_equal(VRPayment::TransactionState::AUTHORIZED, transaction_processed.state,
  ...
  def test_create_complete_immediately
    ...
    transaction_create.tokenization_mode = VRPayment::TokenizationMode::FORCE_CREATION
    transaction_create.customers_presence = VRPayment::CustomersPresence::NOT_PRESENT
    transaction_create.completion_behavior = VRPayment::TransactionCompletionBehavior::COMPLETE_IMMEDIATELY

    transaction = @@transaction_service.create(SPACE_ID, transaction_create)
    transaction_processed = @@card_processing_service.process(SPACE_ID, transaction.id, 
      TEST_CARD_PAYMENT_METHOD_CONFIGURATION_ID, FAKE_CARD_DATA)

    assert_equal(VRPayment::TransactionState::FULFILL, transaction_processed.state,
  ...
  def test_count
    ...
    entity_query_filter = {
      filter: VRPayment::EntityQueryFilter.new({
          fieldName: "id",
          value: transaction.id,
          type: VRPayment::EntityQueryFilterType::LEAF,
          operator: VRPayment::CriteriaOperator::EQUALS
      })
    }
  ...
  def test_read
    ...
    assert_equal(VRPayment::TransactionState::PENDING, transaction_read.state,
      "State must be PENDING")
  end
  ...
  def test_search
    ...
    entity_query = VRPayment::EntityQuery.new({
      filter: VRPayment::EntityQueryFilter.new({
        fieldName: "id",
        value: transaction.id,
        type: VRPayment::EntityQueryFilterType::LEAF,
  ...
  def test_process_one_click_token_and_redirect_with_credentials
    ...
    transaction1_create.tokenization_mode = VRPayment::TokenizationMode::FORCE_CREATION_WITH_ONE_CLICK_PAYMENT
    transaction1_create.customers_presence = VRPayment::CustomersPresence::NOT_PRESENT
    transaction1_create.completion_behavior = VRPayment::TransactionCompletionBehavior::COMPLETE_DEFERRED
    ...
    transaction2_create.tokenization_mode = VRPayment::TokenizationMode::FORCE_CREATION_WITH_ONE_CLICK_PAYMENT
    transaction2_create.customers_presence = VRPayment::CustomersPresence::NOT_PRESENT
    transaction2_create.completion_behavior = VRPayment::TransactionCompletionBehavior::COMPLETE_DEFERRED
    ...
    assert_equal(VRPayment::TransactionState::AUTHORIZED, transaction2_read.state, 
      "State must be AUTHORIZED")

    # cleanup - 2nd transaction
    transaction2_completed = @@transaction_completion_service.complete_online(SPACE_ID, transaction2.id)

    assert_equal(VRPayment::TransactionCompletionState::SUCCESSFUL, transaction2_completed.state,
    ...
    assert_equal(VRPayment::TransactionCompletionState::SUCCESSFUL, transaction1_completed.state,
      "State must be SUCCESSFUL")
